...
- **Setting `sendingTimeout` below the worst-case SMTP roundtrip**: a healthy still-mid-SMTP worker can lose its `isSending` lock to a "recovery" worker, causing a duplicate send. Since v5 `___startClaimRenewal` re-stamps `sendingAt` mid-send (`renewClaim`, bounded by `maxRenewals`), so this is survivable — but `sendingTimeout` still sets crash-recovery latency, and the constructor warns below `120000`. **Do not remove the renewal's lease guard**: it must reuse `__withLease(task, …)` so it succeeds only while this worker still owns the row.
