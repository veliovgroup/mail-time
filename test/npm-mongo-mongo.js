...
const TEST_TITLE = `mail-time-test-suite-mongo-mongo-${Date.now()}`;
    ...
    describe(type, () => {
      ...
      describe('cancelMail', function () {
        ...
        it('sendMail then cancelMail', async function () {
          ...
          const task = await mailTimes[type].queue.collection.findOne({
      ...
      describe('sendMail', function () {
        ...
        it('sendMail Template placeholders render', async function () {
          ...
          const isCancelled = await mailTimes[type].cancelMail(uuid);
          assert.isTrue(isCancelled, 'isCancelled true from cancelMail');
          let qty = await mailTimes[type].queue.collection.countDocuments({
            isCancelled: false,
            'mailOptions.to': address
          });
          assert.equal(qty, 0, 'no tasks after cancel');

          qty = await mailTimes[type].queue.collection.countDocuments({
            'mailOptions.to': address
          });
          if (keepHistory) {
            assert.equal(qty, 1, 'Task saved {keepHistory: true}');
          } else {
            assert.equal(qty, 0, 'Task removed {keepHistory: false}');
          }
        });
        ...
        it('sendMail testing concatenation to the same addressee - different emails', async function () {
          ...
          let tasks = await (await mailTimes[type].queue.collection.find({
            'mailOptions.to': address
          })).toArray();

          let qty = tasks.length;
          ...
          if (concat === true) {
            ...
            assert.equal(tasks[0].mailOptions.length, 2, '{task.mailOptions} hold two emails');
          } else {
            ...
            assert.equal(qty, 1, 'Has one email records after .cancelMail(uuid2)');
          }
          ...
          assert.equal(qty, 0, 'Has no email records after .cancelMail(uuid1)');

          qty = await mailTimes[type].queue.collection.countDocuments({
            'mailOptions.to': address
          });
          if (keepHistory) {
            assert.equal(qty, concat ? 1 : 2, 'Task saved {keepHistory: true}');
          } else {
            assert.equal(qty, 0, 'Task removed {keepHistory: false}');
          }
        });
        ...
        it('sendMail testing concatenation to the same addressee - same emails', async function () {
          ...
          if (concat === true) {
            assert.equal(uuid1, uuid2, 'Email uuids are the same when concatenated');
            assert.equal(qty, 1, 'Has single email record with concatenation');
            assert.equal(tasks[0].mailOptions.length, 1, '{task.mailOptions} hold one emails');
          } else {
            assert.notEqual(uuid1, uuid2, 'Email uuids are notEqual concatenation is disabled');
            assert.equal(qty, 2, 'Has two email records without concatenation');
